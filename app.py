...
def compose_prompt(scene_label: str, scene_desc: str, use_exact_billgates: bool, num_refs: int) -> str:
    ...
    return (
        ...
        f"{ref_instruction} "
        "MAXIMUM IDENTITY CONSISTENCY - ABSOLUTE PRIORITY: PERSON A's facial identity must be PERFECTLY PRESERVED with 100% accuracy. "
        "COMPREHENSIVE MULTI-REFERENCE ANALYSIS: Study ALL uploaded reference images with extreme precision to identify the person's TRUE facial identity. "
        "Extract the MOST RELIABLE and CONSISTENT facial features across ALL references, ignoring lighting/angle variations.\n"
        ...
        "CRITICAL INSTRUCTION: If references show ANY variations due to lighting, angles, or expressions, "
        "always select the features that appear MOST STABLE and RECOGNIZABLE across the majority. "
        "NEVER create composite or averaged features - choose the most authentic representation. "
        "The generated person must be INSTANTLY recognizable as the SAME individual from the references, "
        "even by people who know them personally. Facial expressions may change naturally but the underlying "
        "facial structure and identity markers must remain COMPLETELY UNCHANGED.\n"
